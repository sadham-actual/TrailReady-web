...
function EnlistButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="h-12 whitespace-nowrap rounded-none border border-stone-800 bg-stone-900 px-6 font-mono text-xs uppercase tracking-wider text-stone-100 transition-colors hover:bg-action-orange disabled:cursor-not-allowed disabled:opacity-70"
    >
...
export default function ExpeditionWaitlistPage() {
  ...
  const showMessage = state.status === 'success' || state.status === 'duplicate';
          ...
          <div className="relative border border-stone-800 bg-stone-100 px-6 py-10 sm:px-10">
            ...
            <div className="relative z-10 space-y-8">
              ...
              {showMessage ? (
                ...
                <form action={formAction} className="mx-auto flex w-full max-w-xl flex-col gap-4 sm:flex-row">
              ...
              {state.status === 'error' && (
                <div className="text-center font-mono text-xs uppercase tracking-wider text-action-orange">
                  {state.message}
                </div>
              )}
            </div>
